...
Функция ПодготовитьПроверяемыйТекстНаСервере(ТекстКода)
	...
	Пока ТекущаяСтрока <> Неопределено Цикл // строки читаются до символа перевода строки
...
Процедура КомандаСпеллерТест(Команда)
	...
	КомандаСпеллерТестНаСервере();
	
КонецПроцедуры

&НаКлиенте
Процедура КомандаСпеллерТестНаСервере()
	
	Ответ = "";
	
	СписокСтрок = Новый СписокЗначений;
	СписокСтрок.ЗагрузитьЗначения(СтрРазделить(ТекстКода, Символы.ПС, Ложь));
	...
	Для Каждого ТекущийЭлСписка Из СписокСтрок Цикл
		
		Сч = Сч + СтрДлина(ТекущийЭлСписка.Значение);
		...
		Иначе
			...
			врмСтрок.Добавить(ТекущийЭлСписка.Значение);
